...
func (bc *BlockChain) writeNBlocksWithState(startBlock, block *types.Block, receipts []*types.Receipt, statedb *state.StateDB) error {
	if block.NumberU64()%2 == 0 && startBlock != nil && block.NumberU64() > startBlock.NumberU64() {
	...
	// Must write head due to the blockHash opCode
	rawdb.WriteBlock(blockBatch, block)
	...
	if block.NumberU64()%2 == 0 {
		newroot, err := statedb.Commit(block.NumberU64(), bc.chainConfig.IsEIP158(block.Number()), bc.chainConfig.IsCancun(block.Number(), block.Time()))
		if err != nil {
			return err
		}
		root = newroot
...
func (bc *BlockChain) writeNBlocksAndSetHead(startBlock, block *types.Block, receipts []*types.Receipt, logs []*types.Log, state *state.StateDB, emitHeadEvent bool) (status WriteStatus, err error) {
	if err := bc.writeNBlocksWithState(startBlock, block, receipts, state); err != nil {
		return NonStatTy, err
	}
	currentBlock := bc.CurrentBlock()

	// Reorganise the chain if the parent is not the head block
	if block.NumberU64()%2 == 0 && startBlock != nil {
...
func (bc *BlockChain) InsertChain(chain types.Blocks) (int, error) {
	...
	_, n, err := bc.insertChainWithNBlocks(chain, true, false)
	return n, err
}
...
func (bc *BlockChain) insertChainWithNBlocks(chain types.Blocks, setHead bool, makeWitness bool) (*stateless.Witness, int, error) {
	fmt.Println("insertChainWithNBlocks============")
	// If the chain is terminating, don't even bother starting up.
	if bc.insertStopped() {
		return nil, 0, nil
	}
	...
	// All blocks use the same stateDB to simulate commiting after processing multiple blocks
	startBlock := block
	endBlock := block
	parent := it.previous()
	if parent == nil {
	...
	if err != nil {
		fmt.Println("parentbn:", startBlock.NumberU64()-1)
	...
	for ; block != nil && err == nil || errors.Is(err, ErrKnownBlock); block, err = it.next() {
		...
		parent := parent
		// The traced section of block import.
		res, err := bc.processBlockWithState(parent.Root, block, setHead, makeWitness && len(chain) == 1, statedb)
		if err != nil {
			return nil, it.index, err
		}
	...
	default:
		...
		log.Warn("Inserted block with unknown status", "number", endBlock.Number(), "hash", endBlock.Hash(),
			"diff", endBlock.Difficulty(), "elapsed", common.PrettyDuration(time.Since(startTime)),
...
func (bc *BlockChain) processBlockWithState(parentRoot common.Hash, block *types.Block, setHead bool, makeWitness bool, statedb *state.StateDB) (_ *blockProcessingResult, blockEndErr error) {
	...
	if bc.cacheConfig.TrieCleanNoPrefetch {
		fmt.Println("processBlockWithState======")
	} else {
		fmt.Println("prefetch enabled===========================")
		// If prefetching is enabled, run that against the current state to pre-cache
		// transactions and probabilistically some of the account/storage trie nodes.
	...
	// Must commit block header due to blockHash opCode
	startBlock := bc.GetBlockByNumber(block.NumberU64() - 1)

	if !setHead {
		// Don't set the head, only insert the block
		err = bc.writeNBlocksWithState(startBlock, block, res.Receipts, statedb)
	} else {
		_, err = bc.writeNBlocksAndSetHead(startBlock, block, res.Receipts, res.Logs, statedb, false)
	}
	if err != nil {
		return nil, err
	}
